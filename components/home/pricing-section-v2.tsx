      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[repeat(auto-fit,minmax(260px,1fr))] gap-8 max-w-6xl mx-auto">
          {sortedPlans.map((plan) => (
            <div
              key={plan.id}
              className={`relative bg-white rounded-2xl shadow-md hover:shadow-lg transition-all duration-200 hover:translate-y-[-4px] hover:scale-[1.02] overflow-hidden ${
            ...
            >
              ...
              <div
                className={`absolute top-0 left-0 px-4 py-1 text-white text-sm font-medium ${
                  plan.id === "free" ? "bg-purple-600" : plan.id === "starter" ? "bg-indigo-600" : "bg-gray-400"
                }`}
                style={{
              ...
              {/* Popular Badge */}
              {plan.popular && (
                <div className="absolute -top-4 right-4 bg-purple-600 text-white px-4 py-1 rounded-full text-sm font-medium">
                  Popular
                </div>
              )}
              ...
              <div className="p-6 md:p-8">
                ...
                <div className="mt-6 mb-4">
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <button
                              ...
                              onClick={() => trackEvent("pricing_tooltip_open", { planId: plan.id })}
                  ...
                  {/* Usage note */}
                  {plan.extraChatPrice && (
                    <p className="text-sm text-gray-500 mt-1">+{plan.extraChatPrice} per extra chat</p>
                  )}
                </div>
                ...
                {/* Feature List */}
                <ul className="space-y-3 mb-8 text-sm leading-relaxed list-disc pl-4">
                ...
                {plan.available ? (
                  <Button
                    ...
                    asChild
                    onClick={() => trackEvent("pricing_cta_click", { planId: plan.id, availability: true })}
                    aria-label={`Start Garrio ${plan.name} Plan`}
                  >
                    <Link href={`${plan.ctaLink}?plan=${plan.id}&utm_source=pricing_strip`}>
                      {plan.id === "free" ? "Add for Free" : `Start for ${plan.price}`}
                    </Link>
                  </Button>
                ) : (
                  <Button
                    ...
                    onClick={() => {
                      ...
                      trackEvent("pricing_cta_click", { planId: plan.id, availability: false })
                    }}
                    aria-label={`Join waitlist for Garrio ${plan.name} Plan`}
                  >
                    {plan.id === "professional" ? "Talk to Sales" : "Join Waitlist"}{" "}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          ...
          >
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={closeWaitlist} className="text-gray-400 hover:text-gray-600" aria-label="Close modal">
                <X size={24} />
              </button>
            </div>
            ...
            {isSubmitted ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-600">
                  We'll notify you as soon as the {sortedPlans.find((p) => p.id === waitlistPlan)?.name} plan becomes
                  available.
                </p>
              </div>
            ) : (
              <form onSubmit={handleSubmit}>
                <p className="text-gray-600 mb-4">
                  Be the first to know when our {sortedPlans.find((p) => p.id === waitlistPlan)?.name} plan launches.
                </p>
                <div className="mb-4">
