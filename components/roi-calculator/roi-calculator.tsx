...
export default function RoiCalculator() {
  ...
  const [automationRates, setAutomationRates] = useState<
  ...
  const [results, setResults] = useState<{
    costs: Record<
      string,
      { total: number; agentCost: number; softwareCost: number }
    >;
    ...
    const calculateResults = () => {
      ...
      const calculateGarrioCost = () => {
        ...
        if (chatVolume > 350) {
          ...
          softwareCost = 200 + overageChats * 1.0;
        }
      ...
      const calculateCompetitorCost = (competitor: any) => {
        const humanTickets =
          chatVolume * (1 - automationRates[competitor.slug]);
        const agentHours = humanTickets / ticketsPerHour;
        ...
        if (competitor.seatRequired && competitor.seatPricing) {
          ...
          const cheapestSeat = competitor.seatPricing.sort(
            (a: any, b: any) => a.pricePerSeat - b.pricePerSeat,
          )[0];
          ...
          const eligiblePlans = plans.filter(
            (plan: any) => plan.includedTickets >= chatVolume,
          );

          if (eligiblePlans.length > 0) {
            const cheapestPlan = eligiblePlans.sort(
              (a: any, b: any) => a.base - b.base,
            )[0];
            softwareCost = seatCost + cheapestPlan.base;
          } else {
            const highestPlan = plans.sort(
              (a: any, b: any) => b.includedTickets - a.includedTickets,
            )[0];
            const overageTickets = Math.max(
              0,
              chatVolume - highestPlan.includedTickets,
            );
            softwareCost =
              seatCost +
              highestPlan.base +
              overageTickets * highestPlan.overage;
          }
        } else {
          ...
          if (eligiblePlans.length > 0) {
            ...
            softwareCost =
              highestPlan.base + overageTickets * highestPlan.overage;
          }
        }
      ...
      competitorData.competitors.forEach((competitor) => {
        ...
        roi[competitor.slug] =
          garrioCost > 0 ? (savings[competitor.slug] / garrioCost) * 100 : 0;

        // Calculate hours saved
        const garrioHumanTickets =
          chatVolume * (1 - garrioPlanInfo.effectiveAutomationRate);
        const competitorHumanTickets =
          chatVolume * (1 - automationRates[competitor.slug]);
        const hoursDiff =
          (competitorHumanTickets - garrioHumanTickets) / ticketsPerHour;
  ...
  const handleSliderChange = (value: number[]) => {
    ...
    if (typeof window !== "undefined") {
      console.log("Analytics event: roi_slider_change", {
        chats: value[0],
        savings:
          results.highestCompetitorCost.cost - results.costs["garrio"].total,
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed">
            Get a detailed breakdown of your potential savings with Garrio's
            done-for-you customer support. Compare costs, see time savings, and
            understand your ROI.
          </p>
        </div>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex justify-center mb-8">
            <Tabs
              value={timePeriod}
              onValueChange={(value) =>
                setTimePeriod(value as "monthly" | "annual")
              }
            >
              <TabsList>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
            ...
            <div className="bg-gradient-to-br from-green-50 to-emerald-100 rounded-2xl p-8 border border-green-100 shadow-lg">
                ...
                <div className="space-y-6">
                  <div className="text-center">
                    <div className="text-sm text-green-700 mb-2 font-medium">
                      Save vs{" "}
                      {
                        competitorData.competitors.find(
                          (app) =>
                            app.slug === results.highestCompetitorCost.slug,
                        )?.name
                      }
                    </div>
                    <div className="text-4xl md:text-5xl font-bold text-green-800 mb-2 tabular-nums">
                      {formatCurrency(
                        results.savings[results.highestCompetitorCost.slug] ||
                          0,
                      )}
                    </div>
                    ...
                    <div className="text-sm text-green-600 bg-green-50 px-4 py-2 rounded-lg inline-block">
                      {formatPercentage(
                        results.roi[results.highestCompetitorCost.slug] || 0,
                      )}{" "}
                      ROI
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between bg-green-50 p-3 rounded-lg">
                      ...
                      <span className="font-bold text-green-900">
                        {formatPercentage(
                          (results.garrioPlanInfo?.effectiveAutomationRate ||
                            0.7) * 100,
                        )}
                      </span>
                    </div>
              ...
              <div className="mt-8">
                <Button
                  className="w-full bg-purple-600 hover:bg-purple-700 text-white px-6 py-4 rounded-xl font-semibold text-lg shadow-lg hover:shadow-xl transition-all duration-200 flex items-center justify-center gap-2 group"
                  size="lg"
                  asChild
                >
                  <Link
                  ...
                  >
                    Start Saving{" "}
                    {formatCurrency(
                      results.savings[results.highestCompetitorCost.slug] || 0,
                    )}
              ...
              <div className="flex justify-between items-center mb-8">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900 mb-2">
                    {timePeriod === "monthly" ? "Monthly" : "Annual"} Cost
                    Breakdown
                  </h2>
                  <p className="text-gray-600">
                    Compare total costs across platforms
                  </p>
                </div>
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent className="max-w-xs p-4">
                      <p className="text-sm">
                        Total cost includes both software subscription and human
                        agent time needed to handle non-automated tickets.
                      </p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
                ...
                apps={[
                  {
                    ...
                    name: "Garrio",
                    automationRate:
                      results.garrioPlanInfo?.effectiveAutomationRate || 0.7,
                  },
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <div className="text-gray-700 space-y-2 leading-relaxed">
                  <p>
                    <strong>Industry Benchmarks:</strong> All data sourced from
                    our verified claims document, including 15-minute average
                    interaction times and competitor automation rates.
                  </p>
                  <p>
                    <strong>Real Costs:</strong> Agent costs based on BLS data
                    ($20.59/hour median wage) plus 30% benefits/taxes and 40%
                    management overhead. Software costs from current published
                    pricing.
                  </p>
                  <p>
                    <strong>Transparent Sources:</strong> Automation rates and
                    metrics derived from public case studies, official
                    documentation, and verified customer data.
                  </p>
                </div>
                <div className="mt-4">
                  <Link
