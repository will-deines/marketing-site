...
async function getPostContent() {
  ...
  const mdxContent = await convertMDXToPostContent("the-complete-shopify-seo-guide-2025-optimize-your-store-for-rankings");
...
- **Performance optimization**: Continued speed improvements`
    ...
    cta: {
      text: "Boost Your Shopify SEO Today",
      href: "https://apps.shopify.com/garrio?utm_source=blog&utm_campaign=shopify_seo_guide"
    }
  }
}
...
export default async function BlogPage() {
  const blogPostData = await getPostContent();
  const formattedDate = formatDate(blogPostData.publishDate);
  return <BlogPost post={blogPostData} formattedDate={formattedDate} />;
}