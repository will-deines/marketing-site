...
# Updated for 2025 API specifications with official parameters
...
# Build the JSON payload using correct Imagen 3 format
JSON_PAYLOAD="{"
JSON_PAYLOAD+="\"instances\":[{\"prompt\":\"$PROMPT\"}],"
...
print_status "Model: ${MODEL_NAME:-imagen-4.0-generate-preview-06-06}"

# Make the API request to correct endpoint
API_URL="https://generativelanguage.googleapis.com/v1beta/models/${MODEL_NAME:-imagen-4.0-generate-preview-06-06}:predict"
print_status "Endpoint: $API_URL"

print_status "Making API request..."
...
# Process response (debug removed for clean output)

# Check if we have the predictions array
image_count=$(echo "$response" | jq -r '.predictions | length' 2>/dev/null | tr -d '\n' || echo "0")
...
for i in $(seq 0 $((image_count - 1))); do
    ...
    image_data=$(echo "$response" | jq -r ".predictions[$i].bytesBase64Encoded" 2>/dev/null)
...
echo "Model: ${MODEL_NAME:-imagen-3.0-generate-001}" >> "${OUTPUT_BASE}-info.txt"
